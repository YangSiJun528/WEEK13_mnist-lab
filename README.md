...
2. 브라우저에서 아래 주소로 이동해 노트북을 엽니다. `USERNAME`, 저장소명, 브랜치명은 본인 또는 팀 환경에 맞게 바꿉니다. (브랜치가 `main`이면 URL의 `master`를 `main`으로 변경)
  ```python
    ...
    # 현재 주소
    https://colab.research.google.com/github/YangSiJun528/WEEK13_mnist-lab/blob/main/mnist_lab.ipynb
  ```
...
3일 내에 익히기에는 내 배경지식이 부족한 관계로, colab에서 돌려보기나 좀 해보려고 간단한 워크샵 형태의 자료를 찾아서 해봄. (직접 구현하는 형태는 아님.)  
드롭다운 이후부터는 내용이 좀 달라서 저장은 안함.  
https://codelabs.developers.google.com/codelabs/cloud-tensorflow-mnist?hl=ko#0

그리고 LLM 개발 프로젝트에서는 touch를 쓰므로 지금 하는 오차역전파법이나 활성화 함수 구현을 꼭 알 필요는 없음.
